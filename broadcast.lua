...
local function listener()
   ...
   if rv then
      -- bind successful: we are the first listener on this host
      --
      -- proxy incoming events to local broadcast subscribers
...
local function subscriber()
   ...
   sched(function()
      while sched.running() and not got_ping do
   ...
   while sched.running() and not got_ping and ping_attempts > 0 do
      local msg = msgpack.pack_array({'broadcast.ping', 0})
      broadcast_socket:sendto(msg, local_addr)
      ping_attempts = ping_attempts - 1
      sched.sleep(0.1)
   end
   -- only continue if noone invoked sched.quit() while we were busy
   ...
   -- wait for events in a background thread
   -- (doesn't keep the event loop alive)
   sched.background(function()
      -- net.qpoll() returns on sched.quit()
